...
namespace ve
{
    struct VulkanCommandContext {
        VulkanCommandContext(VulkanMainContext& vmc, VulkanRenderContext& vrc) : vmc(vmc), vrc(vrc), sync(vmc.logical_device.get())
        {
            ...
            buffers.push_back(Buffer(vmc, vertices, vk::BufferUsageFlagBits::eVertexBuffer, {uint32_t(vmc.queues_family_indices.transfer), uint32_t(vmc.queues_family_indices.graphics)}, command_buffers[1][0]));
            buffers.push_back(Buffer(vmc, indices, vk::BufferUsageFlagBits::eIndexBuffer, {uint32_t(vmc.queues_family_indices.transfer), uint32_t(vmc.queues_family_indices.graphics)}, command_buffers[1][0]));
        ...
        void record_graphics_command_buffer(uint32_t idx, uint32_t image_idx)
        {
            ...
            command_buffers[0][idx].bindVertexBuffers(0, buffers[0].get(), offsets);
            command_buffers[0][idx].bindIndexBuffer(buffers[1].get(), 0, vk::IndexType::eUint32);
            command_buffers[0][idx].drawIndexed(buffers[1].get_element_count(), 1, 0, 0, 0);
