...
namespace ve
{
        VulkanRenderContext::VulkanRenderContext(const VulkanMainContext& vmc, VulkanCommandContext& vcc) : vmc(vmc), vcc(vcc), descriptor_set_handler(vmc), surface_format(choose_surface_format(vmc)), render_pass(vmc, surface_format.format), swapchain(vmc, surface_format, render_pass.get()), pipeline(vmc)
        {
        ...
        vk::SurfaceFormatKHR VulkanRenderContext::choose_surface_format(const VulkanMainContext& vmc)
        {
